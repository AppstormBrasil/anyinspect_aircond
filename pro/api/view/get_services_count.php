...
if(isset($_POST['IdUser'])){ $IdUser = $_POST['IdUser'];} else { $IdUser  = '';}
if(isset($_POST['type'])){ $type = $_POST['type'];} else { $type  = '';}

 function get_area_clients($IdUser,$type){
    $response = array();
    $db = new db();
    /////////////////////// SERVICES TYPES CURRENT MONTH ////////////////////////////
    $db->query("SELECT lat , lon FROM tb_client pc ORDER BY lat "); 
    ...
    return $data;
        ...
        // GET MONTH COMPARE 
 function get_month_compare($IdUser,$type){
        $current_month = date('m');
        $current_day = date('d');
        $prev_month = $current_month - 1;
        $response = array();
        ...
        if($type == 'a'){
            ...
            FROM tb_booking pb
            LEFT JOIN tb_book_detail pbd  ON pb.id = pbd.id 
            WHERE ( (MONTH(start_date) = ".$current_month." OR MONTH(start_date) = ".$prev_month.") AND YEAR(start_date) = 2020)  AND pbd.id_funcionario = $IdUser
            GROUP BY MONTH(start_date) , DAY(start_date) ORDER BY DAY(start_date)  "); 
        }
         ...
         if($type == 'a'){
            ...
            LEFT JOIN tb_book_detail pbd  ON pb.id = pbd.id 
            LEFT JOIN tb_services ps ON ps.id = pbd.service_name
            WHERE ( (MONTH(pb.start_date) = ".$current_month." ) AND YEAR(pb.start_date) = 2020) AND pbd.id_funcionario = $IdUser
            GROUP BY ps.id "); 
         }
         ...
         if($type == 'a'){
            ...
            LEFT JOIN tb_book_detail pbd  ON pb.id = pbd.id 
            LEFT JOIN tb_services ps ON ps.id = pbd.service_name
            WHERE ( (MONTH(pb.start_date) = ".$prev_month." ) AND YEAR(pb.start_date) = 2020) AND pbd.id_funcionario = $IdUser
            GROUP BY ps.id "); 
         }
    ...
    $response['produto_total_prev'] = $prodprev;
		 ...
		 function get_valor_ganho_current($IdUser,$type){
			$current_month = date('m');
			$current_day = date('d');
			$prev_month = $current_month - 1;
			$total_gasto = 0;
		 ...
		 function get_valor_ganho_prev($IdUser,$type){
			$current_month = date('m');
			$current_day = date('d');
			$prev_month = $current_month - 1;
			$total_gasto = 0;
        ...
        function get_semanal($IdUser,$type){
            $db = new db(); 
            ...
            if($type == 'a'){
                ...
                FROM tb_booking pb
                LEFT JOIN tb_book_detail pbd  ON pb.id = pbd.id 
                WHERE pbd.id_funcionario = $IdUser 
                GROUP BY DAYNAME(pb.start_date) , MONTH(pb.start_date)
                ORDER BY MONTH(pb.start_date)";
            }
          ...
          function get_servico_mensal($IdUser,$type){
        ...
        function get_clients_visits($IdUser,$type){
            $db = new db();
            if($type == 'a'){
                $db->query("SELECT COUNT(pc.id) as total_visits , pc.name as nome_cliente 
                FROM tb_booking pb
                LEFT JOIN tb_book_detail pbd ON pbd.id_booking = pb.id 
                LEFT JOIN tb_client pc ON pb.id_client = pc.id 
         ...
         $clients_visits =  get_clients_visits($IdUser,$type);
        
        function get_tb_visits($IdUser,$type){
            $db = new db();
         ...
         $tb_visits =  get_tb_visits($IdUser,$type);
        
         function get_service_visits($IdUser,$type){
            $db = new db();
         ...
         $ser_visits =  get_service_visits($IdUser,$type);
         $mensal = get_servico_mensal($IdUser,$type);
         $week_val =  get_semanal($IdUser,$type);
         $compare_val = get_month_compare($IdUser,$type);
         $get_area_clients = get_area_clients($IdUser,$type);
		 ...
		 $get_valor_ganho_current =  get_valor_ganho_current($IdUser,$type);
		 $get_valor_ganho_prev =  get_valor_ganho_prev($IdUser,$type);
