function get_lista_pet(){
	...
	$.ajax({
     url:"includes/view/pet/get_open_service",
	 method:"GET",
	 dataType: 'JSON',
     data:{id:idCliente},
	  ...
	  if(status == "SUCCESS") {
		...
		for(var a = 0; a < lista_pet.length; a++){
			...
			lista_pets += '<tr id="row_pet_'+id_pet+'">'+
												'<td id="nome_pet_'+id_pet+'"><img  style="width:35px;height:35px;border-radius:50%;" class="user_pic" src="assets/images/favicon.png" alt="Arquivo"> '+name+'</td>'+
