...
if($type == 'a' || $type == 'g' || $type == 'r'){
	...
	WHERE DATE(b.start_date) LIKE '%$the_date%' AND b.status <> 'Deletado' GROUP BY b.id ORDER BY b.start_date" ); 
} else {
	...
	LEFT JOIN tb_category tcba ON tca.category = tcba.id 
	LEFT JOIN tb_services ps ON ps.id = bd.service_name
	WHERE DATE(b.start_date) LIKE '%$the_date%' AND b.status <> 'Deletado' AND tbteam.id = $id  GROUP BY b.id ORDER BY b.start_date" ); 
