...
if($id > 0){
	  $db->query("SELECT td.* , taei.IdEnviaComunicado ,
      ...
      WHERE td.IdCondominio ='$id' AND td.tipo = 'Regra Área Comum Restríto' GROUP BY taei.IdEnviaComunicado ORDER BY  td.data_cadastro DESC "); 
 } else { 
  	 $someArray = [];
...
if($result){ 
	 ...
	 foreach ($result as $row) {
		...
		if($status_morador == 0){
			$is_new = '<a href="#/ig/'.$IdComunicadoTemplate.' "><div class="message_type_yellow fr small message_type">Novo</div></a>';
		} else {
			$is_new = '';
		}
		...
		'tipo_mensagem' => 'Informações Gerais',
