...
if($id > 0){
      ...
      WHERE td.IdCondominio ='$id' AND td.tipo = 'Convenção' GROUP BY taei.IdComunicadoTemplate ORDER BY  td.data_cadastro DESC "); 
...
if($result){ 
	 ...
	 foreach ($result as $row) {
		...
		if($status_morador == 0){
			$is_new = '<a href="#/convencoes/'.$IdComunicadoTemplate.' "><div class="message_type_yellow fr small message_type">Novo</div></a>';
		} else {
			$is_new = '';
		}
