...
if($id > 0){
      ...
      WHERE td.IdCondominio ='$id' AND td.tipo = 'Estatuto' GROUP BY td.IdComunicadoTemplate ORDER BY  td.data_cadastro DESC "); 
...
if($result){ 
	 ...
	 foreach ($result as $row) {
		...
		if($status_morador == 0){
			$is_new = '<span><div class="message_type_yellow fr small message_type">Novo</div></span>';
		} else {
			$is_new = '';
		}
