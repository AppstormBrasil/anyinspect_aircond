...
if(isset($_GET['the_date'])){ $the_date = $_GET['the_date'];} else { $the_date  = '';}
$id_cliente = $id;
...
if($type == 'a' || $type == 'g' || $type == 'r' ){ 
	...
	WHERE DATE(b.start_date) LIKE '%$the_date%' AND b.status NOT IN('Deletado')  GROUP BY b.id_group ORDER BY b.start_date" ); 
	$db->execute();
	$result = $db->resultset(); 
} else {
	...
	LEFT JOIN tb_category tcba ON tca.category = tcba.id 
	LEFT JOIN tb_services ps ON ps.id = bd.service_name 
	WHERE (bd.id_funcionario = :id OR tbteam.name = 'Todos' ) AND DATE(b.start_date) LIKE '%$the_date%' AND b.status NOT IN('Deletado') 
...
function get_total_grupo($id_group){
	...
	From tb_booking b 
	WHERE DATE(b.start_date) LIKE '%$the_date%' AND b.status NOT IN('Deletado') AND b.id_group = :id_group 
	GROUP BY b.id_group " );
