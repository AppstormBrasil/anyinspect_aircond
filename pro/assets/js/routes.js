  ...
  },
  {
    ...
    on: {
      ...
      pageAfterIn:function (e, page) {
        window.animatelo.bounceIn('#logo_parceiro');
      },
      pageAfterOut: function (e, page) {
        sync_download();
      },
  ...
  pageInit: function (e, page) {
    ...
    setTimeout(function () {
      ...
      $$('.backs').on('click', function () {
        ...
        var id_hist = "";
        id_hist = '/atividade/'+id_booking+''; 
        hist.forEach(function(value,i) {
          if(value == id_hist){
            view.router.back(view.history[i], {
              force: true
            });
            return;
          } 
    ...
    on: {
      pageInit: function (e, page) {
        ...
        $(".tab" ).on('tab:show', function() {
          ...
          if(tabId == 'tab-1'){
            setTimeout(function () {
              $('.page-content').animate({scrollTop:(scroll_pos)}, '100');
           }, 300);
            $$('.left_tab_act').html('')
              ...
              dummy_manuais += '<li class="col-100 medium-50">'+
                                      '<div class="card-bx job-card">'+
                                          '<div class="card-media">'+
                                              '<a target="_blank" class="link" href="'+link_page+'"><img style="border-radius:50%;" src="img/logo/logo1.png" alt=""/></a>'+
                                          '</div>'+
                                          '<div class="card-info">'+
                                              ...
                                              '<div class="item-footer">'+
                                                  ...
                                                  '<h3 class="item-price">'+data.ref_fabricante+'</h3>'+
                                              '</div>'+
                                          '</div>'+
                                      '</div>'+
                                  '</li>';
  ...
  on: {
      pageInit: function (e, page) {
        ...
        setTimeout(function () {
          ...
          var IdUser = user_info.id;
          app.request.json(current_path + '/view/get_treinamentos',  {IdUser: IdUser } , function (data) {
            myinfo = data;
            var dummy_manuais = "";
            var manual_result = data.data.length;
            $('#manual_results').html(manual_result)
