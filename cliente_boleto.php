...
<div class="container-fluid">
        ...
        <div class="col-lg-12">
            <div class="card invoice-info-card" style="margin: auto;">
                <div class="card-header text-center">
                    <h4>Informações de Pagamento</h4><h1 class="mb-5">MÊS <?php echo $month;?>/<?php echo $year;?></h1>
                </div>
                <div class="card-body">
                    <div class="row mb-5">
    ...
    <script>
        ...
        function get_info_gerais(){
            ...
            $.ajax({
            url:"includes/cliente/get_info_pay_cliente",
            method:"GET",
            dataType: 'JSON',
            data:{
                id:id_func,
                ...
                $("#foto_funcionario").html('<a href="cliente-'+id_funcionario+'"><img  style="width:65px;height:65px;float: left;" class="avatar_table" src="'+foto_funcionario+'" alt=""></a>');
                lista_comissao = "";
                total_pagar = 0;
                if(comissao.length > 0){
                    for(var a = 0; a < comissao.length; a++){
                        ...
                        descricao = comissao[a].descricao;
                        foto_ativo = comissao[a].foto_ativo;
                        status = comissao[a].status;
                        color = comissao[a].color;
                        textColor = comissao[a].textColor;
                    ...
                    lista_comissao += '<tr>'+
                                        '<td><img  style="width:30px;height:30px;float: left;" class="avatar_table" src="'+foto_ativo+'" alt="">'+descricao+'</td>'+
                                        '<td>'+short_dec+'</td>'+
                                        '<td>'+save_date+'</td>'+
                                        '<td class="text-center">R$ <span>'+comission+'</span></td>'+
                                        '<td class="text-center"><span style="background:'+color+';color:'+textColor+'" class="label label label-rounded ">'+status+'</span></td>'+
                                    '</tr>';
                    }
                    ...
                    lista_comissao += '<tr>'+
                                            ...
                                            '<td class="text-center" style="font-weight: 700;font-size: 18px;">R$ <span>'+comission_total+'</span></td>'+
                                        '</tr>';
