<?php
  ...
  if(isset($_POST['id_clientt'])){ $id_clientt = $_POST['id_clientt'];} else {$id_clientt = '';}
  $db = new db(); 
...
tcli.complemento as complemento_cliente , tcli.city as cidade_cliente , tcli.state_ as estado_cliente , tcli.nome_empresa , tcli.lat as lat_cliente , tcli.lon as lon_cliente , tcli.foto as foto_cliente , tcli.nome_empresa
FROM tb_client tcli WHERE tcli.id  = :id ");
$db->bind(':id', $id_clientt);
$result = $db->resultset();
  ...
  $response = array();
	if($result){
		...
		WHERE tt.type = 'r' AND ttq.desc_qual = 'CREA'"); 
		$db->execute();
		$resp_tec = $db->single(); 
