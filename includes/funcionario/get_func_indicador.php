...
$db->query("SELECT 
YEAR(pb.start_date) AS yr, MONTH(pb.start_date) AS m, 
COUNT(pb.start_date) AS total_ , 
SUM(pbd.price) AS soma_total ,
SUM(pc.comission) AS soma_ 
...
LEFT JOIN tb_client pcl ON pcl.id = pb.id_client
WHERE  pc.id_func = :id
...
if($result){
	 ...
	 $db->query("SELECT 
		SUM(pc.comission) AS soma_valor_comissao,
		SUM(ps.price) AS soma_valor_total,
		COUNT(YEAR(pb.start_date)) AS total_geral
		...
		WHERE pbf.id_fun = :id
		GROUP BY YEAR(pb.start_date) ,pb.id"); 
	 $db->bind(':id', $id);
	 $db->execute();
	 ...
	 if($result_total){
		...
		$soma_valor_comissao =  $result_total['soma_valor_comissao'];
	 }
	 ...
	 $db->query("SELECT 
				YEAR(pb.start_date) AS yr, MONTH(pb.start_date) AS m , pb.start_date, 
				pbd.ended_at
				FROM tb_book_detail pbd
				LEFT JOIN tb_booking pb ON pb.id = pbd.id_booking
				LEFT JOIN tb_book_func pbf ON pbf.id_booking = pb.id
				...
				LEFT JOIN tb_comission pc ON pc.id_func = pbf.id_fun
				WHERE pbf.id_fun = :id
				GROUP BY pb.id ORDER BY pbd.ended_at DESC LIMIT 1 "); 
				$db->bind(':id', $id);
				$db->execute();
				$result_total = $db->single(); 
				if($result_total){
					$ended_at =  $result_total['ended_at'];
					if($ended_at){
						$latest_visit = usa_to_br_date_time($ended_at);
					}
				}
	  ...
	  LEFT JOIN tb_services ps ON ps.id = pbd.service_name
	  LEFT JOIN tb_client pcl ON pcl.id = pb.id_client
	  /*LEFT JOIN tb_client pcl ON pcl.id = pcp.id_client*/
	  WHERE  pc.id_func = :id
	  GROUP BY pb.id ORDER BY count(ps.short_dec) DESC LIMIT 1  "); 
	  $db->bind(':id', $id);
	  $db->execute();
	  $result_total = $db->single(); 
	  if($result_total){
	  ...
	  function get_semanal(){
		...
		if(isset($_POST['id'])){ $id = $_POST['id'];} else {$id = '';}
		$db->query("SELECT 
		count(ps.id) AS total_ , 
		pbd.price as soma_total,
		YEAR(pb.start_date) AS yr, MONTH(pb.start_date) AS m , pb.start_date, DAYNAME(pb.start_date) as dia_semana
		FROM tb_book_detail pbd
		LEFT JOIN tb_booking pb ON pb.id = pbd.id_booking
		LEFT JOIN tb_book_func pbf ON pbf.id_booking = pb.id
		LEFT JOIN tb_services ps ON ps.id = pbd.service_name
		...
		if($result){
			...
			foreach($result as $row) {
				...
				if($m == 1){
					...
					if($dia_semana == 'Monday'){
						$dez_seg = array();
						array_push($dez_seg, 10, 0, $dum_dez);
					} else if($dia_semana == 'Tuesday'){
						$dez_ter = array();
						array_push($dez_ter, 10, 1, $dum_dez);
					} else if($dia_semana == 'Wednesday'){
						$dez_qua = array();
						array_push($dez_qua, 10, 2, $dum_dez);
					} else if($dia_semana == 'Thursday'){
						$dez_qui = array();
						array_push($dez_qui, 10, 3, $dum_dez);
					} else if($dia_semana == 'Friday'){
						$dez_sex = array();
						array_push($dez_sex, 10, 4, $dum_dez);
					} else if($dia_semana == 'Saturday'){
						$dez_sab = array();
						array_push($dez_sab, 10, 5, $dum_dez);
					} else if($dia_semana == 'Sunday'){
						$dez_dom = array();
						array_push($dez_dom, 10, 6, $dum_dez);
					}
				}
	 ...
	 $db->query("SELECT 
	 COUNT(ps.id) as total_service, ps.short_dec ,  pbd.price as price_total 
	 FROM tb_book_detail pbd
	 LEFT JOIN tb_booking pb ON pb.id = pbd.id_booking
     LEFT JOIN tb_book_func pbf ON pbf.id_booking = pb.id
	 LEFT JOIN tb_services ps ON ps.id = pbd.service_name
	 LEFT JOIN tb_team pt ON pt.id = pbd.id_funcionario
	 WHERE pbf.id_fun = :id GROUP BY ps.id  "); 
	 $db->bind(':id', $id);
	 $db->execute();
	 $result = $db->resultset(); 
	 $dataprev = array();
	 ...
	 //////////////////// SERVICES TYPES PREVIEW MONTH //////////////////////////
	 $db->query("SELECT 
	 SUM(TIME_TO_SEC(TIMEDIFF(pbd.ended_at,pbd.started_at))) as total_gasto,
	 SUM(TIME_TO_SEC(ps.est_time)) as total_esperado
	 FROM tb_book_detail pbd
	 LEFT JOIN tb_booking pb ON pb.id = pbd.id_booking
     LEFT JOIN tb_book_func pbf ON pbf.id_booking = pb.id
	 LEFT JOIN tb_services ps ON ps.id = pbd.service_name
	 ...
	 if($result){
		 foreach($result as $row) {
			 ...
			 $hora_total_esperado =  gmdate('H:i:s', $row["total_esperado"]);
			 //$month_ = get_nome_mes($row["month_"]);

			//$aux = array("name" => $total_gasto, "y" => (int)$total_esperado);	
			//array_push($horaespe, $aux);
			
		 }
		 //$arr['hora_total_gasto'] = (float)$hora_total_gasto;
		 //$arr['hora_total_esperado'] = (float)$hora_total_esperado;
		 
		 $arr['hora_total_gasto'] = $hora_total_gasto;
		 $arr['hora_total_esperado'] = $hora_total_esperado;
	...
	$arr['dataprev_prod'] = 0;
	$arr['valor_total_prod_mes_prev'] = 0;
	...
	$arr['soma_valor_comissao'] = $soma_valor_comissao;
