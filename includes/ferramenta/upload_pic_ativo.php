  ...
  $name = $id.'.jpg';
	...
	if (isset($_FILES['upload_file'])) {
      ...
      if(is_array($_FILES)) {
          ...
          try{
            $db = new db(); 
            $db->query('UPDATE tb_clients_ativo SET foto = :foto  WHERE id = :id ');
            $db->bind(':foto', $name);
            $db->bind(':id', $id); 		 
            if($db->execute()){ 
              $arr['status'] = "SUCCESS";
            ...
            $arr['status_txt'] = "Não foi possível atualizar a imagem , verifique o se o tamanho está abaixo de  1,5Mb";
            $arr['status'] = "ERROR";
            echo json_encode($arr);
            exit(0);
          }
      ...
      $arr['status_txt'] = "Não foi possível atualizar a imagem , se o problema persistir entre em contato com o Administrador";
      $arr['status'] = "ERROR";
      echo json_encode($arr);
      exit(0);
    }
