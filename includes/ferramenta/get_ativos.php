...
$db->query("SELECT tca.descricao , tca.foto as foto_ativo, tca.id as id_ativo , tcat.description as categoria , tca.local ,   tcat.id , tcl.foto as foto_client , tcl.name as nome_client , tcl.id as id_client , tcl.phone as phone_client , tl.descricao as descricao_local , DATE_FORMAT(tca.validade,'%d/%m/%Y' ) as data_validade , tca.model , tca.register , tcl.nome_empresa , tca.patrimonio, tca.sn, tca.pn, tca.calibracao , DATE_FORMAT(tca.calibracao,'%d/%m/%Y' ) as calibracao , tca.tipo , ta.iata 
FROM tb_tooling tca
...
LEFT JOIN tb_aeroportos ta ON ta.iata = tca.base  "); 
$db->execute();
...
if($result){
	 ...
	 foreach($result as $row) {
		...
		$response['data'][] = array(
			...
			"botao"=>'<a style="margin-right: 5px;" class="single_link btn btn-primary btn-xs" id="'.$id_ativo.'" href="ferramenta-'.$id_ativo.'">
