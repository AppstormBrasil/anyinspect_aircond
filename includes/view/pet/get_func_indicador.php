...
$db->query("SELECT YEAR(pb.start_date) AS yr, MONTH(pb.start_date) AS m, 
			...
			SUM(round(((pbd.price * pt.comission) /100 ) ,2)) as soma_
			FROM pet_booking pb
...
if($result){
	 ...
	 $db->query("SELECT YEAR(pb.start_date) AS yr, MONTH(pb.start_date) AS m, 
				...
				SUM(pbd.price) AS soma_valor_total_ ,
				SUM(round(((pbd.price * pt.comission) /100 ) ,2)) as soma_valor_total
				FROM pet_booking pb
				LEFT JOIN pet_book_detail pbd ON pbd.id_booking = pb.id 
				LEFT JOIN pet_client pc ON pb.id_client = pc.id 
				LEFT JOIN pet_clients_pet pcp ON pcp.id_client = pc.id 
	 ...
	 $db->query("SELECT YEAR(pb.start_date) AS yr, MONTH(pb.start_date) AS m , pb.start_date
				...
				LEFT JOIN pet_services ps ON ps.id = pbd.service_name 
				LEFT JOIN pet_team pt ON pt.id = pbd.id_quem_executou
				WHERE pt.id = :id AND pb.status = 'FINALIZADO' "); 
	 $db->bind(':id', $id);
	 $db->execute();
	 $result_total = $db->single(); 
	 if($result_total){
	  ...
	  function get_semanal(){
		$db = new db(); 
		if(isset($_POST['id'])){ $id = $_POST['id'];} else {$id = '';}
		$db->query("SELECT count(ps.id) AS total_ , 
	 ...
	 $db->query("SELECT count(pb.id) total_service ,
				ps.id as id_servico, ps.short_dec ,  
				pbd.price as price_total ,
				SUM(round(((pbd.price * pt.comission) /100 ) ,2)) as price				
				FROM pet_booking pb
				LEFT JOIN pet_book_detail pbd ON pbd.id_booking = pb.id 
				LEFT JOIN pet_client pc ON pb.id_client = pc.id 
				LEFT JOIN pet_clients_pet pcp ON pcp.id_client = pc.id 
