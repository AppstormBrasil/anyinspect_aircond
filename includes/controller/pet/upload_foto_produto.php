...
if (isset($_FILES['ufile_image_product'])) {
    $id = $_POST['id_productt'];
    $max_file_size = 1024*10000;
   
    $file = $_FILES['ufile_image_product'];

    $path = '../../../images/pet/upload/produtos/';
    ...
    if ($file['size'] > $max_file_size) {
        ...
        if(move_uploaded_file($_FILES['ufile_image_product']['tmp_name'], $path . $new_name)){
            try{
                $db = new db(); 
                $db->query('UPDATE pet_product SET picture = :imagem  WHERE id = :id ');
                $db->bind(':imagem', $name);
                $db->bind(':id', $id); 		 
                if($db->execute()){ 
                    $arr['status'] = "SUCCESS";
					...
					header( "Refresh:3; url=http://localhost/Agendazy/pet-lista-produto-detalhado.php?id=$id", true, 303);
					exit(0);
                } else {
                $arr['status'] = "ERROR";
