...
$db->query('SELECT id, name, cpf, rg, phone, phone2, email, neighbor, city , foto , insta_cliente , nome_empresa from tb_client'); 
$db->execute();
...
if($result){
	 ...
	 foreach($result as $row) {
		...
		$response['data'][] = array(
			...
			//"all_pets"=>$all_pets,
			"botao"=>'<a style="margin-right: 5px;" class="single_link btn btn-light btn-xs" id="'.$row['id'].'" href="cliente-'.$id.'">
