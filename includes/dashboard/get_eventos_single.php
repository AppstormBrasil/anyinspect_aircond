...
pc.name as nome_cliente , pc.nome_empresa ,  tcba.description as category , pc.zip, pc.street, pc.number, pc.neighbor, pc.complemento, pc.city, pc.state_, pc.lat, pc.lon
FROM tb_booking b 
  ...
  $endereco_cliente = "";
	if($result){
		foreach($result as $row) {
			...
			$response[] = array(
				"id"=>$id_evento,
				"title"=>$id_client.'-['.$descricao.' <b>'.$category.'</b> ] '.$nome_cliente,
