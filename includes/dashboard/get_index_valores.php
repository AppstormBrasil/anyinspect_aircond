...
$db->query("SELECT count(id) as count_id from tb_booking"); 
$db->execute();
$result = $db->resultset(); 
if($result){
	foreach($result as $row) {
...
$db->query("SELECT count(id) as count_id from tb_booking where start_date like '%$data_hoje%' and status ='Finalizado' AND status <> 'Deletado'"); 
$db->execute();
$result = $db->resultset(); 
if($result){
	foreach($result as $row) {
