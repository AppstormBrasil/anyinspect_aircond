$(document).ready(function(){
    ...
    var calendar = $('#calendar_').fullCalendar({ 
        ...
        resourceRender: function(resourceObj,labelTds, bodyTds) {
            ...
            labelTds.html('<div style="'+elStyles+'">\
                      <div style="'+imageStyles+'">\
                      	</div><div style="line-height:40px;"><a style="color:#fff;" class="single_link" href="funcionario-'+resourceObj.id+' ">'+resourceObj.title+'</a></div>\
                    </div>')
          },
        ...
        if(id_ativo == 0 || id_ativo == null || id_ativo == 'null'){
            ...
            box_ativo = '<a class="single_link" href="ativo-'+id_ativo+'"><span  ><img class="avatar_table" src="'+foto_ativo+'" alt="Avatar" height="30" width="30"></a><span><strong>'+ativo+'</strong></span> </span>'
        }
        ...
        <div class="row mb-4">
			...
			<div class="col-8" id="name_client_edit" ><a class="single_link" href="cliente-`+id_client+`"><span><img class="avatar_table" src="`+foto_cliente+`" alt="Avatar" height="30" width="30"></a><span style="margin-right: 10px;">` +name_client+`</span></span>
			</div>
		</div>
        <div class="row mb-4">
            <div class="col-4">
            ...
            <div class="col-8" id="name_pet_edit">
            `+box_ativo+`
			</div>
		</div>    
        <div class="row mb-4">
    ...
    $('.edit_pet').on('click', function(e){ 
        ...
        $('#pet_cliente_edit').select2({
            ...
            $('.update_event').on('click', function(e){ 
                ...
                var valor_interface = '<a class="single_link" href="barco-'+valor_campo+'"><span><img class="avatar_table" src="'+foto+'" alt="Avatar" height="30" width="30"></a><span><strong>'+name+'</strong></span>';
    ...
    $('.edit_client').on('click', function(e){ 
            ...
            '<div id="modalBodyTime" class="modal-body">'+
                ...
                '<div class="col-lg-12">'+
                    '<div class="form-group">'+
                        ...
                        '<div class="input-group">'+
                            '<select style="width: 100%;height:70px;border: 1px solid #dddfe1;" id="cliente_edit_" name="cliente_edit_" required></select>'+
                        '</div>'+
                    '</div>'+
                '</div>'+
                ...
                '<div class="col-lg-12">'+
                    '<div class="form-group">'+
                        '<label class="text-label">Ativo/Equipamento</label>'+
                        '<div class="input-group">'+
                            '<select style="width: 100%;height:70px;border: 1px solid #dddfe1;" id="pet_cliente_edit_" name="pet_cliente_edit_" required></select>'+
                        '</div>'+
                    '</div>'+
                '</div>'+
                '<input type="hidden" id="id_edit" value="'+id+'" />'+
        ...
        $('#pet_cliente_edit_').select2();

        $('#cliente_edit_').select2({
            ajax: {
        ...
        $('#cliente_edit_').on('select2:select', function (e) {
                e.preventDefault();
                var id_cliente = $('#cliente_edit_').val();

                $("#pet_cliente_edit_").val('').trigger('change');
                $('#pet_cliente_edit_ option:selected').remove();
                

                console.log('to aqui' + id_cliente)
        ...
        $('.update_event').on('click', function(e){ 
            ...
            var choose_pet = $('#pet_cliente_edit_').select2('data');
            var choose_client = $('#cliente_edit_').select2('data');
            let id_event = $('#id_edit').val();
            ...
            var valor_interface_client = '<a class="single_link" href="cliente-'+valor_campo+'"><span><img class="avatar_table" src="'+foto_client+'" alt="Avatar" height="30" width="30"></a><span><strong>'+name_client+'</strong></span>';
        ...
        $('#lista_funcionario_filtro_edit').select2({
            ...
            $('.update_event').on('click', function(e){ 
                ...
                for(let i = 0; i < choose_pet.length; i = i + 1 ) {
                    valor_interface += '<a class="single_link" href="ativo-'+choose_pet[i].id+'"><span><img class="avatar_table" src="'+choose_pet[i].foto+'" alt="Avatar" height="30" width="30"></a><span><strong>'+choose_pet[i].name+'</strong></span>';
    ...
    $('.edit_servico').on('click', function(e){ 
        ...
        function servicesearch(data) {
            ...
            if (data.id == 'none') {
                return 'Nenhuma Atividade atribuído <a style="float: right;" class="single_link btn btn-primary btn-sm" href="funcionario-'+id_team +'" >Atribuir Atividade</a>';
            } else {
                ...
                $('#has_chose_fun').val(resourceObj.resourceId);
				   ...
				   choose_fun = '<div class="col-lg-12">'+
							'<div class="form-group">'+
								'<label class="text-label">Funcionário</label>'+
								'<div class="input-group">'+
								'<strong><a class="single_link" href="funcionario-'+resourceObj.id+'"><span><img style="width:40px;height:40px;margin-right: 10px;" class="avatar_table" src="'+resourceObj.foto+'" alt="Avatar" height="45" width="45"></span>'+resourceObj.name+'</a></strong>'+
								'</div>'+
							'</div>'+
						'</div>';
...
function doDelete(){  
 ...
 function saveCalendary(){ // add event
        ...
        if(tipo_servico != ''){} else {
			toastr.error('Erro!', "Oops, Escolha a atividade!");
			return;
        }
       ...
       $.ajax({
        url: "includes/calendario/cadastra_agendamento_air",
       data: {
        startTime:startTime,
        endTime:endTime,
        clientes:clientes,
                           ...
                           data: {
                            ...
                            repprove_message:repprove_message
                                ...
                                success: function(response) {
                                    ...
                                    toastr.success("Atividade finalizado com sucesso!", 'Sucesso');
                                    $('#calendarModal').modal('hide');
                                    $('#calendar_').fullCalendar( 'refetchEvents' );
                                    $('#concluido').hide();
                                    $('#reprovar').hide();
			...
			if(acao == "concluido"){
				...
				information = '<div class="user-info">'+
                        ...
                        '<div class="detail">'+
							...
							'<h5>O Atividade foi finalizada! Deseja enviar uma mensagem no whatsapp do cliente?</h5>'+
                ...
                toastr.success("Atividade iniciado com sucesso!", 'Sucesso');
                $('#calendarModal').modal('hide');
				setTimeout(function(){ 
                    $('#calendar_').fullCalendar( 'refetchEvents' );
                 }, 100);
